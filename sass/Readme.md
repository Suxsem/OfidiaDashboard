...
folder are mapped to the application's root namespace, 'OfidiaDashboard'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "OfidiaDashboard"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'OfidiaDashboard' in the same way
as `"OfidiaDashboard/sass/src"`.
